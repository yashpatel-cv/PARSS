...
# === INTERACTIVE CONFIGURATION VARIABLES (NEW DEFAULTS) ===
...
phase_4_luks_encryption() {
    ...
    echo -n "$luks_passphrase" | execute_cmd \
        ...
        "Initializing LUKS2 on home partition" true
    
    log_info "Opening encrypted home volume..."
    echo -n "$luks_passphrase" | execute_cmd \
        "cryptsetup luksOpen $HOME_PARTITION $LUKS_HOME_NAME -" \
        "Opening LUKS volume as $LUKS_HOME_NAME" true
    
    if [[ ! -b "/dev/mapper/$LUKS_HOME_NAME" ]]; then
        log_error "Failed to open encrypted home volume"
        return 1
    fi
